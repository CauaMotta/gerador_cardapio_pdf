...
for %%p in ("Git.Git" "OpenJS.NodeJS.LTS") do (
    echo Instalando %%p...
    winget install -e --id %%~p --accept-source-agreements --accept-package-agreements
    echo.
)

set /p INSTALL_SUBLIME="Deseja instalar o Sublime Text? (S/N): "

if /I "%INSTALL_SUBLIME%"=="S" (
    echo Instalando Sublime Text...
    winget install -e --id "SublimeHQ.SublimeText.4" --accept-source-agreements --accept-package-agreements
    echo.
    ...
    echo Pulando instalacao do Sublime Text.
)
